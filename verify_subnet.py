...
def get_subnet(subnet):
    ...
    for ip in ip:
        try:
            ...
            hosttype=host.split('.')[0]
            unique=list()
            clustertype=[]
            clustertype='.'.join(host.split('.')[2:6])
    ...
    try:
        ...
        sz=netinfo.split(":")[4]
        if '70' or '88' not in sz:
                if 'api' or 'mq' or 'db' or 'quantum' or 'glance' or 'nova' or 'keystone' in hosttype:
                    print "FLAG: wrong host types"
        if '60' or '86' not in sz:
                if 'hv' in hosttype:
                    print "FLAG: wrong host types"
